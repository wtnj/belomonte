...
±±ºPrograma  ³OIFR003   ºAutor  ³Leandro Ribeiro     º Data ³  13/06/14   º±±
...
User Function OIFR003()  

Local _aArea := GetArea() 
Local cPerg  := Padr("OIFR003",Len(SX1->X1_GRUPO))      	             
                                                                                      //AL2A                                                                                                     ENG
//PutSx1(cPerg,"01","Tipo de Saldo"       , "", "", "mv_ch1" , "C", 08,0 ,0, "C", "", ""      ,"", "", "MV_PAR01", "PG" , "" , "" , "", "RE"      ,"","","EM","","","CT","","","","","",""/*,"","",""*///)     
PutSx1(cPerg,"01","Saldo Realizado"       , "", "", "mv_ch1" , "C", TAMSX3("AKD_TPSALD")[1],0 ,0, "C", "", ""    ,"", "", "MV_PAR01", "PR" , "" , "" , "" , "EM" , "" , "" , "RE" , "" , "" , "PG" , "","","PR+EM+RE","","","","","","","",""   ,"")
PutSx1(cPerg,"02","Saldo Orçado"          , "", "", "mv_ch2" , "C", TAMSX3("AKD_TPSALD")[1],0 ,0, "C", "", ""    ,"", "", "MV_PAR02", "CT" , "" , "" , "" , "OR" , "" , "" , "OR+CT" , "" , "" , "" , "","","","","","","","","","",""   ,"")
...
PutSx1(cPerg,"09","Data De"                  , "", "", "mv_ch9"  , "D", TAMSX3("AKD_DATA")[1]  ,0, 0, "G", "", ""    ,"", "", "MV_PAR09", "" , "" , "" , "" , "" , "" , "" , "" , "" , "" , "" , "","","","","","","","","","",""   ,"")
PutSx1(cPerg,"10","Data Ate"                 , "", "", "mv_ch10" , "D", TAMSX3("AKD_DATA")[1]  ,0, 0, "G", "", ""    ,"", "", "MV_PAR10", "" , "" , "" , "" , "" , "" , "" , "" , "" , "" , "" , "","","","","","","","","","",""   ,"")
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÉÍÍÍÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍ»±±
±±ºPrograma  ³GOIFR003   ºAutor  ³Leandro Ribeiro    º Data ³  13/06/14   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
...
Static Function GOIFR003()

Local _cOIFR003  := GetArea()
//Local _cFile     := "OIFR003" + DToS( Date() ) + ".xml"                      
Local _cFile     := "OIFR003.xml"                      
...
Local _cCont     := 0
Local _cRetSoma  := ""  
Local _cMesDe    := ReturnMes(MV_PAR09)    
Local _cMesAte   := ReturnMes(MV_PAR10) 
Local _cTotal    := 0
Local _cConta    := 0 
//Local _cUDia	 := SUBSTR(DTOS(MV_PAR08),1,6)+AllTrim(STR(Last_Day(DTOS(MV_PAR08))))

//Private _cUltimoD  := SUBSTR(DTOS(MV_PAR08),1,6)+AllTrim(STR(Last_Day(SUBSTR(DTOS(MV_PAR08),1,6)+"01")))
//Private _cFirstD   := SUBSTR(DTOS(MV_PAR08),1,6)+"01"                   
Private _cUltimoD  := DTOS(MV_PAR10)
Private _cFirstD   := DTOS(MV_PAR09)
Private _cMV_PAR01 := ""
Private _cMV_PAR02 := ""
Private _xMV_PAR01 := ""
Private _xMV_PAR02 := ""
...
/*If(DTOS(MV_PAR09) > _cUDia)
	Aviso("Aviso","Data excede "+SUBSTR(_cUDia,7,8)+" dias, favor verificar os parametros!",{"OK"})
		U_OIFR003()
		Return()
EndIf*/
...
_cQuery1 += " AND AKD_CO BETWEEN '"+MV_PAR11+"' AND '"+MV_PAR12+"' "+ c_ent  
...
fWrite(_nHandle,'    <Cell ss:StyleID="s65"><Data ss:Type="String">Periodo de '+DTOC(MV_PAR09)+' a '+DTOC(MV_PAR10)+'</Data></Cell>'+ c_ent)
fWrite(_nHandle,'    <Cell ss:StyleID="s67"/>'+ c_ent)
fWrite(_nHandle,'    <Cell ss:StyleID="s64"/>'+ c_ent)
fWrite(_nHandle,'    <Cell ss:StyleID="s64"/>'+ c_ent)
fWrite(_nHandle,'    <Cell ss:StyleID="s66"/>'+ c_ent)
...
fWrite(_nHandle,'    <Cell ss:StyleID="s68"/>'+ c_ent)
fWrite(_nHandle,'    <Cell ss:StyleID="s69"><Data ss:Type="String">Tipo Saldo 1</Data><Comment'+ c_ent)
fWrite(_nHandle,'      ss:Author="TOTVS"><ss:Data'+ c_ent)
...
fWrite(_nHandle,'    <Cell ss:StyleID="s69"><Data ss:Type="String">Tipo Saldo 2</Data><Comment'+ c_ent)
fWrite(_nHandle,'      ss:Author="TOTVS"><ss:Data'+ c_ent)
fWrite(_nHandle,'       xmlns="http://www.w3.org/TR/REC-html40"><B><Font html:Face="Tahoma"'+ c_ent)
fWrite(_nHandle,'         x:Family="Swiss" html:Size="9" html:Color="#000000">selecionar o mes</Font></B></ss:Data></Comment></Cell>'+ c_ent)
fWrite(_nHandle,'    <Cell ss:MergeDown="1" ss:StyleID="s69"><Data ss:Type="String">Variacao</Data></Cell>'+ c_ent)
fWrite(_nHandle,'   </Row>'+ c_ent)
fWrite(_nHandle,'   <Row ss:AutoFitHeight="0">'+ c_ent)
fWrite(_nHandle,'    <Cell ss:StyleID="s63"/>'+ c_ent)
fWrite(_nHandle,'    <Cell ss:StyleID="s68"/>'+ c_ent)
fWrite(_nHandle,'    <Cell ss:StyleID="s68"/>'+ c_ent)
fWrite(_nHandle,'    <Cell ss:StyleID="s69"><Data ss:Type="String">'+_xMV_PAR01+'</Data></Cell>'+ c_ent)
fWrite(_nHandle,'    <Cell ss:StyleID="s69"><Data ss:Type="String">'+_xMV_PAR02+'</Data></Cell>'+ c_ent)
fWrite(_nHandle,'   </Row>'+ c_ent)
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////         
			...
			For _zz := 1 to Len(_cArray2)
				...
				Aadd(_cArray5,AllTrim(CONTREAL(_cArray2[_zz][1],_cArray1[_cc])))
				Aadd(_cArray5,AllTrim(CONTORC(_cArray2[_zz][1],_cArray1[_cc])))
				Aadd(_cArray5,AllTrim(CONTSALD(_cArray2[_zz][1],_cArray1[_cc])))
			
			
				fWrite(_nHandle,'   <Row ss:AutoFitHeight="0">'+ c_ent)
				fWrite(_nHandle,'    <Cell ss:StyleID="s63"/>'+ c_ent)
				fWrite(_nHandle,'    <Cell ss:StyleID="s75"><Data ss:Type="String">'+TiraGraf(Alltrim(POSICIONE("CTD",1,xFilial("CTD")+PADR(_cArray2[_zz][1],TAMSX3("CTD_ITEM")[1]),"CTD_DESC01")))+'</Data></Cell>'+ c_ent)
//				fWrite(_nHandle,'    <Cell ss:StyleID="s75"/>'+ c_ent)
				...
				fWrite(_nHandle,'    <Cell ss:StyleID="s76"><Data ss:Type="Number">0.00</Data></Cell>'+ c_ent)
				fWrite(_nHandle,'   </Row>'+ c_ent)		
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÉÍÍÍÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍ»±±
±±ºPrograma  ³OIFR003   ºAutor  ³Leandro Ribeiro     º Data ³  19/06/14   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
...
//_cQuery3 := " SELECT DISTINCT AKD_ITCTB, AKD_CC " + c_ent
_cQuery3 := " SELECT DISTINCT AKD_ITCTB " + c_ent
...
_cQuery3 += " AND AKD_CO BETWEEN '"+MV_PAR11+"' AND '"+MV_PAR12+"' "+ c_ent  
...
If(!Eof())
  While !(_cTrab3)->(Eof())  
		//Aadd(_cArray3,{AllTrim((_cTrab3)->AKD_ITCTB),AllTrim((_cTrab3)->AKD_CC)})
		Aadd(_cArray3,{AllTrim((_cTrab3)->AKD_ITCTB)})
  	DbSkip()
  EndDo 
EndIf  
...
Static Function CONTREAL(_cItemCTB,_cCoSup)
...
_cQuery4 += " AND AKD_CO BETWEEN '"+MV_PAR11+"' AND '"+MV_PAR12+"' "+ c_ent  
_cQuery4 += " AND AKD_CC BETWEEN '"+MV_PAR05+"' AND '"+MV_PAR06+"' "+ c_ent
_cQuery4 += " AND AKD_STATUS = '1'"+ c_ent
_cQuery4 += " AND AKD_COSUP = '"+_cCoSup+"'"+ c_ent
...
Static Function CONTORC(_cItemCTB,_cCoSup)
...
_cQuery6 += " AND AKD_ITCTB = '"+_cItemCont+"'"+ c_ent
...
_cQuery6 += " AND AKD_CO BETWEEN '"+MV_PAR11+"' AND '"+MV_PAR12+"' "+ c_ent  
_cQuery6 += " AND AKD_CC BETWEEN '"+MV_PAR05+"' AND '"+MV_PAR06+"' "+ c_ent
_cQuery6 += " AND AKD_STATUS = '1'"+ c_ent  
_cQuery6 += " AND AKD_COSUP = '"+_cCoSup+"'"+ c_ent
...
Static Function CONTSALD(_cItemCTB,_cCoSup)
...
_cQuery7 += " AND AKD_ITCTB = '"+_cItemCont+"'"+ c_ent
...
_cQuery7 += " AND AKD_CO BETWEEN '"+MV_PAR11+"' AND '"+MV_PAR12+"' "+ c_ent  
_cQuery7 += " AND AKD_CC BETWEEN '"+MV_PAR05+"' AND '"+MV_PAR06+"' "+ c_ent
_cQuery7 += " AND AKD_STATUS = '1'"+ c_ent   
_cQuery7 += " AND AKD_COSUP = '"+_cCoSup+"'"+ c_ent
...
±±º          ³                                                            º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºUso       ³ AP //NÃO UTILIZADO, MANTINDO PARA FUTURA UTILIZAÇÃO        º±±
±±ÈÍÍÍÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¼±±
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß
*/
...
_cQuery5 += " AND AKD_CO BETWEEN '"+MV_PAR11+"' AND '"+MV_PAR12+"' "+ c_ent  
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÉÍÍÍÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍ»±±
±±ºPrograma  ³OIFR003   ºAutor  ³Leandro Ribeiro     º Data ³  20/06/14   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÉÍÍÍÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍ»±±
±±ºPrograma  ³OIFR003   ºAutor  ³Microsiga           º Data ³  06/21/14   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDesc.     ³                                                            º±±
±±º          ³                                                            º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÉÍÍÍÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍ»±±
±±ºPrograma  ³OIFR003   ºAutor  ³Leandro Ribeiro     º Data ³  28/05/14   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
